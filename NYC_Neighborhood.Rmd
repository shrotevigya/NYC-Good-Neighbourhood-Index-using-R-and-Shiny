---
title: "NYC Good Neighborhood index"
author: "Ujjawal, Meghana, Vigya, Sohil"
date: "May 1, 2018"
output:
  html_document:
    keep_md: true
...
accidents <- read.csv("NYPD_Motor_Vehicle_Collisions.csv", stringsAsFactors = FALSE)

service_requests <- read.csv("service_requests.csv", stringsAsFactors = FALSE)

zipcode <- read.csv("Zip_Neighborhood.csv", stringsAsFactors = FALSE)

neighborhood_zips <- read.csv("Zip_Neighborhood.csv", stringsAsFactors = FALSE)

restaurants<- read.csv("NYC_restaurants_by_type.csv", stringsAsFactors = FALSE)

hospitals<- read.csv("NYC_Health_and_Hospitals_Corporation_Facilities.csv")

school <- shapefile("Public_Schools_Points_2011-2012A.shp")

mta<- read.csv("subways_zip.csv")
...
library(geojsonio)
s <- geojson_read("nyu_2451_34509-geojson.json", what = "sp", string)
...
# read the nyc data
nyc_crime_zips <- read.csv("nyc_crime_zips.csv", stringsAsFactors = FALSE)

# agrregate the crime data by zip code
#nyc_crime_zips<-aggregate(OFNS_DESC ~ postal_code, nyc_crime_zips, length)

nyc_crime_neighborhoods <- merge(nyc_crime_zips, neighborhood_zips, by.x='postal_code_new', by.y='Zipcode')
...
```{r message=FALSE, warning=FALSE}
s <- geojson_read("nyu_2451_34509-geojson.json", what = "sp", string)
s@data <- left_join(s@data %>% mutate(postal_code = as.numeric(as.character(zcta))),
                      nyc_crime_neighborhoods %>%
                        group_by(postal_code) %>%
                        summarize(Num_of_Crimes = n()),
                      by = "postal_code")

s_labels <- paste("<b>Zip Code:</b>",s$postal_code,"<br/>",
                    "<b>No. of Crimes:</b>", s$Num_of_Crimes,"<br/>")
...
crime_map <- leaflet(nyc_crime_neighborhoods, options = leafletOptions(minZoom = 10)) %>%
  ...
  addLegend("bottomright",
            pal = colorNumeric("RdYlGn", s$Num_of_Crimes, reverse = TRUE), 
            values = ~s$Num_of_Crimes,
            title = "Num of Crimes",
            opacity = 1)
...
s <- geojson_read("nyu_2451_34509-geojson.json", what = "sp", string)
s@data <- left_join(s@data %>% mutate(IncidentZip = as.numeric(as.character(zcta))),
                      service_requests_n_zip %>%
                        group_by(IncidentZip) %>%
                        summarize(Total_Number_of_Complaints = n()),
                      by = "IncidentZip")

s_labels <- paste("<b>Zip Code:</b>",s$IncidentZip,"<br/>",
                  "<b>Total number of Complaints:</b>", s$Total_Number_of_Complaints,"<br/>")
...
map_final <- leaflet(service_requests_n_zip, options = leafletOptions(minZoom = 10)) %>%
  ...
  addPolygons(data = s, stroke = TRUE, smoothFactor = 0.2,
              ...
              popup = s_labels) %>%
  addLegend("topleft",
            pal = colorNumeric("RdYlGn", s$Total_Number_of_Complaints, reverse = TRUE), 
            values = ~s$Total_Number_of_Complaints,
...
ggplot(mta_top, aes(x=neighborhood, y=n, fill= political) ) +
  geom_bar( stat = "identity")+ facet_wrap(~political,nrow = 1, scales = "free_x")+theme(axis.text.x = element_text(angle=90, size=rel(0.8), hjust=1))+labs(title = "Neighborhoods with most MTA Stations",y = "", x = "")+scale_fill_manual(values=c("#d3ffce", "#FFCCAC", "#FFEB94","#C1E1DC","#FDD475"))
```
